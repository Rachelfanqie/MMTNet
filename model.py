...
class ReConv2d(torch.nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
...
class ConvProject(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, kernel_size=3, q_stride=1, k_stride=1, v_stride=1, dropout = 0.,
        ...
        self.to_q = ReConv2d(dim, inner_dim, kernel_size, q_stride, pad, bias)
        self.to_k = ReConv2d(dim, inner_dim, kernel_size, k_stride, pad, bias)
        self.to_v = ReConv2d(dim, inner_dim, kernel_size, v_stride, pad, bias)
...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size,num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        if token_projection =='conv':
            self.qkv = ConvProject(dim,num_heads,dim//num_heads,bias=qkv_bias)
...
#########################################
########### LEWTransformer #############
class LEWTransformerBlock(nn.Module):
...
class MFEM(nn.Module):
    def __init__(self, out_plane, act_layer=nn.ReLU):
        super(MFEM, self).__init__()
        self.conv11 = nn.Sequential(
            BasicConv(3, out_plane // 4, kernel_size=1, stride=1, relu=False))
        self.conv12 = nn.Sequential(
            BasicConv(out_plane // 4, out_plane // 2, kernel_size=1, stride=1, relu=False))
        self.dconv1 = nn.Sequential(
            nn.Conv2d(out_plane//4, out_plane//4, groups=out_plane//4, kernel_size=3, stride=1, padding=1),
            act_layer())
        self.dconv2 = nn.Sequential(
            nn.Conv2d(out_plane//2, out_plane//2, groups=out_plane//2, kernel_size=3, stride=1, padding=1),
            act_layer())
    ...
    def forward(self, x):
        ...
        x3 = self.conv(x3)
        B, C, H, W = x3.shape
        x3 = x3.flatten(2).transpose(1, 2).contiguous()  # B H*W C
        return x3
...
class FAM(nn.Module):
    def __init__(self, channel, act_layer=nn.Sigmoid):
        super(FAM, self).__init__()
        self.conv1 = BasicConv(channel, channel, kernel_size=1, stride=1, relu=False)
        self.conv2 = nn.Sequential(
    ...
    def forward(self, x1, x2):
        ...
        H1 = int(math.sqrt(L1))
        W1 = int(math.sqrt(L1))
        x1 = x1.transpose(1, 2).view(B1, C1, H1, W1)
        B2, L2, C2 = x2.shape
        H2= int(math.sqrt(L2))
        W2 = int(math.sqrt(L2))
        x2 = x2.transpose(1, 2).view(B2, C2, H2, W2)
        x = torch.cat([x1, x2], dim=1)
        x = self.conv2(x)
        y = self.conv1(x1)
        y = x * y
        out = y + self.conv1(x1) + self.conv1(x2)
        B, C, H, W = out.shape
        out = out.flatten(2).transpose(1, 2).contiguous()  # B H*W C
        return out

#########################################
########### Basic layer of MMTNet ################
    ...
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        if shift_flag:
            self.blocks = nn.ModuleList([
                LEWTransformerBlock(dim=dim, input_resolution=input_resolution,
...
class MMTNet(nn.Module):
    def __init__(self, in_chans=3, dd_in=3, img_size=256,
                 ...
                 cross_modulator=False, **kwargs):       # num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
        super().__init__()
        ...
        # Input/Output
        self.input_proj = InputProject(in_channel=dd_in, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.output_proj = OutputProject(in_channel=2*embed_dim, out_channel=in_chans, kernel_size=3, stride=1)

        # conv
        self.feat_extract = nn.ModuleList([
            BasicConv(3, embed_dim, kernel_size=3, relu=True, stride=1),
            BasicConv(embed_dim, embed_dim * 2, kernel_size=3, relu=True, stride=2),
            BasicConv(embed_dim * 2, embed_dim * 4, kernel_size=3, relu=True, stride=2),
            BasicConv(embed_dim * 4, embed_dim * 2, kernel_size=4, relu=True, stride=2, transpose=True),
            BasicConv(embed_dim * 2, embed_dim, kernel_size=4, relu=True, stride=2, transpose=True),
            BasicConv(embed_dim, 3, kernel_size=3, relu=False, stride=1)
        ])
        ...
        self.AFFs = nn.ModuleList([
            AFF(embed_dim * 7, embed_dim * 1),
            AFF(embed_dim * 7, embed_dim * 2)
        ])

        self.FAM1 = FAM(embed_dim * 2)
        self.MFEM1 = MFEM(embed_dim * 2)
        self.FAM2 = FAM(embed_dim * 4)
        self.MFEM2 = MFEM(embed_dim * 4)
        self.FAM3 = FAM(embed_dim * 8)
        self.MFEM3 = MFEM(embed_dim * 8)
    ...
    def forward(self, x, mask=None):
        ...
        y = self.input_proj(x)
        y = self.pos_drop(y)
        y_2 = F.interpolate(x, scale_factor=0.5)
        y2 = self.MFEM1(y_2)   #128
        y_3 = F.interpolate(y_2, scale_factor=0.5)
        y3 = self.MFEM2(y_3)   #64
        y_4 = F.interpolate(y_3, scale_factor=0.5)
        y4 = self.MFEM3(y_4)   #32

        #Encoder
        conv0 = self.encoderlayer_0(y,mask=mask)
        pool0 = self.dowsample_0(conv0)
        pool0 = self.FAM1(pool0, y2)

        conv1 = self.encoderlayer_1(pool0,mask=mask)
        pool1 = self.dowsample_1(conv1)
        pool1 = self.FAM2(pool1, y3)

        conv2 = self.encoderlayer_2(pool1,mask=mask)
        pool2 = self.dowsample_2(conv2)
        pool2 = self.FAM3(pool2, y4)
...
if __name__ == "__main__":
    ...
    model_restoration = MMTNet(img_size=input_size, embed_dim=32,depths=depths,
