...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, img_options=None, target_transform=None):
        ...
        self.tar_size = len(self.clean_filenames)  # noisy_filenames
        # self.tar_size_noisy = len(self.noisy_filenames)
        # self.tar_size_clean = len(self.clean_filenames)# get the size of target
    ...
    def __getitem__(self, index):
        ...
        # tar_index_clean = index % self.tar_size_clean
        # tar_index_noisy = index % self.tar_size_noisy

        clean = torch.from_numpy(np.float32(load_img(self.clean_filenames[tar_index])))
        noisy = torch.from_numpy(np.float32(load_img(self.noisy_filenames[tar_index])))
        # clean = torch.from_numpy(np.float32(load_img(self.clean_filenames[tar_index_clean])))
        # noisy = torch.from_numpy(np.float32(load_img(self.noisy_filenames[tar_index_noisy])))
        ...
        # clean_filename = os.path.split(self.clean_filenames[tar_index_clean])[-1]
        # noisy_filename = os.path.split(self.noisy_filenames[tar_index_noisy])[-1]
...
class DataLoaderVal_deblur(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tar_img = tar_img.resize((512, 512))
